...
puzzle1 = grid((
    ('*', locs(26, 2)),
    ('G', locs(9, 2)),
    ('Y', locs(14, 3, N)),  # N used for cars oriented up-down; it will be passed in for incr in locs()
    ('P', locs(17, 3, N)),
    ('O', locs(41, 2, N)),
    ('B', locs(20, 3, N)),
    ('A', locs(45, 2))))
...
def shortest_path_search(start, successors, is_goal):
    ...
    while frontier:
        path = frontier.pop(0)
        s = path[-1]
        for (state, action) in sorted(successors(s).items(), key=lambda t: abs(t[1][1])):
            state = tuple(sorted(state, key=lambda t: t[0]))
...
def path_states(path): return path[0::2]


### TO DO
# create successors function (state1 -> {state2: action}) to be used by shortest_path_search
# successors fn will return {state: action} where action is the action taken to get to state
# states will be provided by grid()

# action is a pair (object, distance_moved), such as ('B', 16) to move 'B' two squares down on the N=8 grid

# create is_goal function for shortest_path_search to tell whether * is in @

# problem: (initial state, goal location for **) -> shortest solution path (state -> action -> state...)
# ** must overlap @ for goal to be achieved
# An action is a move by one car in one direction (by any number of spaces)
# You will write the function solve_parking_puzzle(start, N=N)
# where 'start' is the initial state of the puzzle and 'N' is the length of a side
# of the square that encloses the pieces (including the walls, so N=8 here)
# The wall in the upper left has index 0 and the one in the lower right has 63.
# We represent a state of the problem with one big tuple of (object, locations)
...
#     up-down
#     left-right
#     moving one, two, three, or more spaces
# path_actions
# spaces
#     cars
#     empty
...
# ** will always be in the same column or row of @ (depending
# on whether ** is oriented up-down or left-right)
...
#  ('B', (20, 28, 36)), 
#  ('A', (45, 46)), 
#  ('|', (0, 1, 2, 3, 4, 5, 6, 7, 8, 15, 16, 23, 31, 32, 39,
#         40, 47, 48, 55, 56, 57, 58, 59, 60, 61, 62, 63)))

# def c(x): print x; return x

# path = solve_parking_puzzle(puzzle4, N=8)
# #print path_actions(path) == [('A', -3), ('B', 16), ('Y', 24), ('*', 4)]
# print path_actions(path)
# for i,s in enumerate(path_states(path)):
#     print
